...
export interface TripData {
  selectedDestination?: { name?: string; state?: string };
  inputs?: { month: number; days: number; groupType: string; pace: string; budget: number };
  itinerary?: {
    summary: string;
    days: {
      day: number;
...
export default function TripItineraryLayout({ trip }: { trip: TripData }) {
      ...
      <div className="bg-hero-gradient pt-24 pb-12">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 text-white">
          ...
          <div className="flex flex-wrap gap-4 mt-4 text-white/70 text-sm">
            <span className="flex items-center gap-1.5"><Calendar className="w-4 h-4" /> {MONTHS[inputs?.month || 0]}</span>
